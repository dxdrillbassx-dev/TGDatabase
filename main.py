...
bot = Bot(token="dxdrillbassx")  # введи сюда токен бота
dp = Dispatcher(bot)
...
allowed_user_ids = [1234567890]  # введи сюда айди пользователей, которым будет доступен функционал бота. Можно несколько людей.
...
async def start(message: types.Message):
    if message.from_user.id in allowed_user_ids:
        ...
        await message.reply("Привет! Это бот для хранения паролей от dxdrillbassx. Чтобы загрузить данные, нажми кнопку 'Загрузить данные'. "
        ...
        await bot.send_photo(message.chat.id, "https://tenor.com/ibHmBTYtzEf.gif")
    else:
